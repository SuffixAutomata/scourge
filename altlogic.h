...
struct cacheKey {
  ...
  const bool operator< (const cacheKey& o) const {
    return std::make_tuple(enforce, remember, row, useless, wid) <
           std::make_tuple(o.enforce, o.remember, o.row, o.useless, o.wid);
  }
};

std::mutex cacheMutex;

std::map<cacheKey, uint64_t**> builtCaches;
...
uint64_t* constructCacheIndex(const cacheKey& k, uint64_t r1, uint64_t r2) {
  ...
  // ...
  // DEBUG << k.row << ' ' << k.enforce << ' ' << k.remember << ' ' << k.useless << ' ' << evxCnt << '\n';
  return res;
}

// int altlogic_brq = 0;

inline std::pair<uint64_t*, int> qryLookup(const cacheKey& k, uint64_t** rows, uint64_t r1, uint64_t r2, uint64_t r3) {
  ...
  if(i == nullptr) {
    ...
    // altlogic_brq++;
    i = constructCacheIndex(k, r1, r2);
  }
  r3 >>= k.useless;
  return {i + i[r3], (int)(i[r3 + 1] - i[r3])};
}
...
void genNextRows(std::vector<uint64_t> &state, int depth, int ahead,
  ...
  // DEBUG << enforce << ' ' << remember << ' ' << enf1 << ' ' << rem1 << ' ' << enf2 << ' ' << rem2 << '\n';
  auto gck = [&](int depth) {
    return std::make_pair(cacheKey{enf_A, rem_A, depth, __builtin_ctzll(rem_A), __builtin_popcountll(rem_A)},
                          cacheKey{enf_B, rem_B, depth, __builtin_ctzll(rem_B), __builtin_popcountll(rem_B)});
  };
  auto gckc = [&](const cacheKey& k) { return getCache(k); };
  const auto & [ck1_A, ck1_B] = gck(depth);
  auto ckc1_A = gckc(ck1_A), ckc1_B = gckc(ck1_B);
  auto [h1_A, hn1_A] = qryLookup(ck1_A, ckc1_A, state[0], state[p], state[C]);
  auto [h1_B, hn1_B] = qryLookup(ck1_B, ckc1_B, state[0], state[p], state[C]);
  // DEBUG << state[0] << ' ' << state[p] << ' ' << state[C] << ' ' << h1n << ' ' << h2n << '\n';
  // std::vector<uint64_t> targ1;
  if(phase >= 3) {
    /*
            x
    */
    ...
    auto [h3_A, hn3_A] = qryLookup(ck3_A, ckc3_A, state[p - 2], state[2 * p - 2], state[2 * p - 1]);
    auto [h3_B, hn3_B] = qryLookup(ck3_B, ckc3_B, state[p - 2], state[2 * p - 2], state[2 * p - 1]);
    auto [h4_A, hn4_A] = qryLookup(ck4_A, ckc4_A, state[p - 3], state[2 * p - 3], state[2 * p - 2]);
    auto [h4_B, hn4_B] = qryLookup(ck4_B, ckc4_B, state[p - 3], state[2 * p - 3], state[2 * p - 2]);
    for (int i = 0; i < hn1_A; i++)
      for (int j = 0; j < hn1_B; j++)
        if ((h1_A[i] & rem_B) == (h1_B[j] & rem_A)) {
          uint64_t r1 = h1_A[i] | h1_B[j];
          if([&]() {
            auto [h2_A, hn2_A] = qryLookup(ck2_A, ckc2_A, state[p - 1], state[2 * p - 1], r1);
            auto [h2_B, hn2_B] = qryLookup(ck2_B, ckc2_B, state[p - 1], state[2 * p - 1], r1);
                        ...
                        for (int i4 = 0; i4 < hn4_A; i4++)
                          for (int j4 = 0; j4 < hn4_B; j4++)
                            if ((h4_A[i4] & rem_B) == (h4_B[j4] & rem_A)) {
                              ...
                              auto [h5_A, hn5_A] = qryLookup(ck5_A, ckc5_A, state[2 * p - 2], r3, r2);
                              auto [h5_B, hn5_B] = qryLookup(ck5_B, ckc5_B, state[2 * p - 2], r3, r2);
                              for (int i5 = 0; i5 < hn5_A; i5++)
                                for (int j5 = 0; j5 < hn5_B; j5++)
                                  if ((h5_A[i5] & rem_B) == (h5_B[j5] & rem_A)) {
                                    uint64_t r5 = h5_A[i5] | h5_B[j5];
                                    auto [h6_A, hn6_A] = qryLookup(ck6_A, ckc6_A, state[2 * p - 3], r4, r3);
                                    auto [h6_B, hn6_B] = qryLookup(ck6_B, ckc6_B, state[2 * p - 3], r4, r3);
                                    for (int i6 = 0; i6 < hn6_A; i6++)
                                      for (int j6 = 0; j6 < hn6_B; j6++)
                                        if ((h6_A[i6] & rem_B) == (h6_B[j6] & rem_A)) {
                                          uint64_t r6 = h6_A[i6] | h6_B[j6];
                                          auto [h7_A, hn7_A] = qryLookup(ck7_A, ckc7_A, r4, r6, r5);
                                          auto [h7_B, hn7_B] = qryLookup(ck7_B, ckc7_B, r4, r6, r5);
                                          for (int i7 = 0; i7 < hn7_A; i7++)
                                            for (int j7 = 0; j7 < hn7_B; j7++)
                                              if ((h7_A[i7] & rem_B) == (h7_B[j7] & rem_A))
                                                return true;
                                        }
                                  }
                            }
          ...
          // fn(h1[i] | h2[j]);//, targ1.push_back(h1[i] | h2[j]);
        }
  } else if (phase == 2) {
    const auto & [ck2_A, ck2_B] = gck(depth + p - 1);
    auto ckc2_A = gckc(ck2_A), ckc2_B = gckc(ck2_B);
                ...
                if ((h2_A[i2] & rem_B) == (h2_B[j2] & rem_A)) {
                  ...
                  for (int i3 = 0; i3 < hn3_A; i3++)
                    for (int j3 = 0; j3 < hn3_B; j3++)
                      if ((h3_A[i3] & rem_B) == (h3_B[j3] & rem_A)) {
                        uint64_t r3 = h3_A[i3] | h3_B[j3];
                        auto [h4_A, hn4_A] = qryLookup(ck4_A, ckc4_A, state[2 * p - 2], r3, r2);
                        auto [h4_B, hn4_B] = qryLookup(ck4_B, ckc4_B, state[2 * p - 2], r3, r2);
                        for (int i4 = 0; i4 < hn4_A; i4++)
                          for (int j4 = 0; j4 < hn4_B; j4++)
                            if ((h4_A[i4] & rem_B) == (h4_B[j4] & rem_A))
                              return true;
    ...
    auto ckc5_A = gckc(ck5_A), ckc5_B = gckc(ck5_B);
    auto [h2_A, hn2_A] = qryLookup(ck2_A, ckc2_A, state[p - 2], state[2 * p - 2], state[p - 1]);
    auto [h2_B, hn2_B] = qryLookup(ck2_B, ckc2_B, state[p - 2], state[2 * p - 2], state[p - 1]);
    for (int i = 0; i < hn1_A; i++)
      for (int j = 0; j < hn1_B; j++)
        if ((h1_A[i] & rem_B) == (h1_B[j] & rem_A)) {
          uint64_t r1 = h1_A[i] | h1_B[j];
          if([&]() {
            for (int i2 = 0; i2 < hn2_A; i2++)
              for (int j2 = 0; j2 < hn2_B; j2++)
                if ((h2_A[i2] & rem_B) == (h2_B[j2] & rem_A)) {
                  uint64_t r2 = h2_A[i2] | h2_B[j2];
                  auto [h3_A, hn3_A] = qryLookup(ck3_A, ckc3_A, state[p - 1], state[2 * p - 1], r1);
                  auto [h3_B, hn3_B] = qryLookup(ck3_B, ckc3_B, state[p - 1], state[2 * p - 1], r1);
                  for (int i3 = 0; i3 < hn3_A; i3++)
                    for (int j3 = 0; j3 < hn3_B; j3++)
                      if ((h3_A[i3] & rem_B) == (h3_B[j3] & rem_A)) {
                        uint64_t r3 = h3_A[i3] | h3_B[j3];
                        auto [h4_A, hn4_A] = qryLookup(ck4_A, ckc4_A, state[2 * p - 2], r2, state[2 * p - 1]);
                        auto [h4_B, hn4_B] = qryLookup(ck4_B, ckc4_B, state[2 * p - 2], r2, state[2 * p - 1]);
                        for (int i4 = 0; i4 < hn4_A; i4++)
                          for (int j4 = 0; j4 < hn4_B; j4++)
                            if ((h4_A[i4] & rem_B) == (h4_B[j4] & rem_A)) {
                              uint64_t r4 = h4_A[i4] | h4_B[j4];
                              auto [h5_A, hn5_A] = qryLookup(ck5_A, ckc5_A, r2, r4, r3);
                              auto [h5_B, hn5_B] = qryLookup(ck5_B, ckc5_B, r2, r4, r3);
                              for (int i5 = 0; i5 < hn5_A; i5++)
                                for (int j5 = 0; j5 < hn5_B; j5++)
                                  if ((h5_A[i5] & rem_B) == (h5_B[j5] & rem_A))
                                    return true;
    ...
    auto ckc6_A = gckc(ck6_A), ckc6_B = gckc(ck6_B);
    auto [h2_A, hn2_A] = qryLookup(ck2_A, ckc2_A, state[p - 1], state[2 * p - 1], state[p]);
    auto [h2_B, hn2_B] = qryLookup(ck2_B, ckc2_B, state[p - 1], state[2 * p - 1], state[p]);
    auto [h3_A, hn3_A] = qryLookup(ck3_A, ckc3_A, state[p - 2], state[2 * p - 2], state[2 * p - 1]);
    auto [h3_B, hn3_B] = qryLookup(ck3_B, ckc3_B, state[p - 2], state[2 * p - 2], state[2 * p - 1]);
    for (int i = 0; i < hn1_A; i++)
      for (int j = 0; j < hn1_B; j++)
                ...
                if ((h2_A[i2] & rem_B) == (h2_B[j2] & rem_A)) {
                  ...
                  for (int i3 = 0; i3 < hn3_A; i3++)
                    for (int j3 = 0; j3 < hn3_B; j3++)
                      if ((h3_A[i3] & rem_B) == (h3_B[j3] & rem_A)) {
                        uint64_t r3 = h3_A[i3] | h3_B[j3];
                        auto [h4_A, hn4_A] = qryLookup(ck4_A, ckc4_A, state[2 * p - 1], r2, r1);
                        auto [h4_B, hn4_B] = qryLookup(ck4_B, ckc4_B, state[2 * p - 1], r2, r1);
                        for (int i4 = 0; i4 < hn4_A; i4++)
                          for (int j4 = 0; j4 < hn4_B; j4++)
                            if ((h4_A[i4] & rem_B) == (h4_B[j4] & rem_A)) {
                              uint64_t r4 = h4_A[i4] | h4_B[j4];
                              ...
                              for (int i5 = 0; i5 < hn5_A; i5++)
                                for (int j5 = 0; j5 < hn5_B; j5++)
                                  if ((h5_A[i5] & rem_B) == (h5_B[j5] & rem_A)) {
                                    uint64_t r5 = h5_A[i5] | h5_B[j5];
                                    auto [h6_A, hn6_A] = qryLookup(ck6_A, ckc6_A, r3, r5, r4);
                                    auto [h6_B, hn6_B] = qryLookup(ck6_B, ckc6_B, r3, r5, r4);
                                    for (int i6 = 0; i6 < hn6_A; i6++)
                                      for (int j6 = 0; j6 < hn6_B; j6++)
                                        if ((h6_A[i6] & rem_B) == (h6_B[j6] & rem_A))
                                          return true;
  ...
  // uint64_t dd;
  // bool alt = false;
  // auto get = [&](int row, int j) {
  //   int realRow = (row + depth - 2 * p) / p;
  //   if (j <= -1 && (realRow < (int)leftborder[-j-1].size()))
  //     return (leftborder[-j-1][realRow] & (1ull << ((row + phase) % p))) ? 1 : 0;
  //   if (j <= -1 || j >= width)
  //     return 0;
  //   if (row < sz(state))
  //     return +!!(((alt && realRow < 2) ? exInitrow[row + depth - 2 * p] : state[row]) & (1ull << j));
  //   assert(row == 2 * p);
  //   return +!!(dd & (1ull << j));
  // };
  // uint64_t enumask = 0;
  // uint64_t rv = remember;
  // if((depth) / p < filters.size()) {
  //   rv &= filters[(depth) / p];
  // }
  // if(sym) {
  //   for (int j=0; j<width; j++) 
  //     if ((j <= width - j - 1) && (rv & (1ull << j)))
  //       enumask |= (1ull << j);
  // } else {
  //   enumask = rv;
  // }
  // std::vector<uint64_t> targ2;
  // for (uint64_t x = enumask; ; x = (x-1) & enumask) {
  //   dd = 0;
  //   if(sym) {
  //     for (int j=0; j<width; j++)
  //       if (remember & (1ull << j))
  //         dd |= ((uint64_t)(+!!(x & (1ull << min(j, width - j - 1)))) << j);
  //   } else {
  //     dd = x;
  //   }
  //   // Check A B dd -> C
  //   for (int j=-1; j<=width; j++)
  //     if (enforce & (1ull << (j + 1))) {
  //       std::vector<int> ar = {get(0, j - 1), get(0, j), get(0, j + 1),
  //                              get(p, j - 1), get(p, j), get(p, j+1),
  //                              get(2*p, j - 1), get(2*p, j), get(2*p, j+1), get(C, j)};
  //       int s = 0;
  //       for (int i = 0; i < 10; i++) if (ar[i]) s |= (1 << i);
  //       if (!table[s])
  //         goto fail;
  //       if (exInitrow.size()) {
  //         alt = true;
  //         std::vector<int> a2 = {get(0, j - 1), get(0, j), get(0, j + 1),
  //                                get(p, j - 1), get(p, j), get(p, j+1),
  //                                get(2*p, j - 1), get(2*p, j), get(2*p, j+1), get(C, j)};
  //         alt = false;
  //         s = 0;
  ...
  //     break;
  // }
  // std::sort(targ1.begin(), targ1.end());
  // std::sort(targ2.begin(), targ2.end());
  // assert(targ1 == targ2);
}
}; // namespace _logic
